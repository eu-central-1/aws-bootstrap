...
class ZipAssetCode(AssetCode):
    ...
    def __init__(self, work_dir: Path, include: Path, file_name: str) -> None:
        """
        :param include: List of packages to include in the lambda archive.
        :param file_name: Lambda ZIP archive name.
        """
        asset_path = LambdaPackaging(include_path=include, work_dir=work_dir, out_file=file_name).package()
...
class LambdaPackaging:
    ...
    def __init__(self, include_path: Path, work_dir: Path = Path(__file__).resolve().parent, out_file: str = str(uuid.uuid4())[:8]) -> None:
    ...
    def _prepare_build(self) -> None:
        ...
        print(f'Copying dependency requirements: {Path(self.requirements_dir) / self.requirements_txt}')
        print(f'    -  {Path(self._include_path) / self.requirements_txt}')
        src = (Path.cwd() / self._include_path / self.requirements_txt).resolve()
        dst = self.requirements_dir / self.requirements_txt
    ...
    def _package_lambda(self) -> None:
        ...
        print(f'Copying \'include\' resources: {self.build_dir}')
        for item in Path(self._include_path).iterdir():
            src = (Path.cwd() / item).resolve()
            dst = self.build_dir / item.name
            if src.is_dir():
                print(f'    -  {item}/')
                shutil.copytree(str(src), str(dst))
            elif item.name != self.requirements_txt:
                print(f'    -  {item}')
                shutil.copy2(str(src), str(dst))
